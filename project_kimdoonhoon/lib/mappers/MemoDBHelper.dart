...
// 생성결과 Map
  ...
  // 북마크 전체 조회
  Future<List<Memo>> getSearch(String search) async {
    Database db = await database; //   Database? _database; 이 (?)부분과 뭔가 관련 있음.
    String sql = "SELECT * FROM memos WHERE info like '%$search%'";
    if (search == null) {
      sql = "SELECT * FROM memos";
    }
    List<Map<String, dynamic>> result =
        // await db.rawQuery("SELECT * FROM memos WHERE info like '%$search%'");
